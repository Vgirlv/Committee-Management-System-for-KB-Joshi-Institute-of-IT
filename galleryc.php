...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['image'])) {
    ...
    if (move_uploaded_file($image_tmp, $target_file)) {
        $stmt = $conn->prepare("INSERT INTO galleryc (image_path) VALUES (?)");
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $stmt = $conn->prepare("SELECT image_path FROM galleryc WHERE id = ?");
    ...
    if ($image_path && unlink($image_path)) {
        $stmt = $conn->prepare("DELETE FROM galleryc WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();
    }
...
<body>
    <div class="container mt-5">
        ...
        <form action="galleryc.php" method="POST" enctype="multipart/form-data" class="mb-3">
        ...
        <div class="row">
            <?php
            $result = $conn->query("SELECT * FROM galleryc ORDER BY id DESC");
                ...
                echo '<div class="col-md-4 mb-3">
                        <div class="card">
                            ...
                            <div class="card-body text-center">
                                <a href="galleryc.php?delete=' . $row['id'] . '" class="btn btn-danger btn-sm">Delete</a>
                            </div>
                        </div>
                    </div>
