...
if (isset($_POST['add_memberr'])) {
    $name = $_POST['name'];
    $role = $_POST['role'];
    
    $query = "INSERT INTO memberss (name, role) VALUES (?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ss", $name, $role);
    $stmt->execute();
    $stmt->close();
    header("Location: memberr.php");
    exit();
}

// Fetch existing members
$query = "SELECT * FROM memberss ORDER BY id DESC";
    ...
    <div class="container mt-5 bg-success">
        ...
        <div class="card p-4 shadow-sm mt-4">
            ...
            <form method="POST">
                ...
                <button type="submit" name="add_memberr" class="btn btn-primary">Add Member</button>
            </form>
        </div>
        <h1>   </h1>
        <a href="cdashboard.php" class="btn btn-light">Back to Dashboard</a>
        </div>
