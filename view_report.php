...
if (isset($_GET['id'])) {
    ...
    // Fetch fund details
    $fund_query = "SELECT * FROM event_expenses WHERE event_id = $event_id";
    $fund_result = mysqli_query($conn, $fund_query);
    $fund = mysqli_fetch_assoc($fund_result);

    // Fetch existing report details (if any)
    $report_query = "SELECT * FROM event_reports WHERE event_id = $event_id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($report) {
        ...
        // Insert new report
        $insert_query = "INSERT INTO event_reports 
    ...
    echo "<script>alert('Report Updated Successfully!'); window.location.href='view_report.php?id=$event_id';</script>";
}
?>
...
<div class="container mt-4">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <a href="view_final_report.php?id=<?php echo $event_id; ?>" class="btn btn-success mt-3">
    View Final Report
</a>
